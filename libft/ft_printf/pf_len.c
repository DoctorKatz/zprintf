...
{
	(ft_findchr(lst->settings, '+')) ? ft_strcpy(opt.sign, "+") : 0;
	(ft_findchr(lst->var, '-')) ? ft_strcpy(opt.sign, "-") : 0;
	(ft_findchr(lst->var, '-') || ft_findchr(lst->settings, '+')) ?
		opt.nb_sig = 1 : 0;
	return (opt);
}
...
t_opt	pf_len_f(t_printf *lst, t_opt opt)
{
	...
	if (ft_findchr(lst->settings, ' ') && !opt.sign[0] && !opt.nb_sp)
...
t_opt	pf_lenu(t_printf *lst, t_opt opt)
{
	...
	if (ft_findchr(lst->settings, '#') && (lst->type == 'x' || lst->type == 'X')
			&& *lst->var != '0')
		opt.nb_zero = 2;
	if (ft_findchr(lst->settings, '#') && (lst->type == 'O' || lst->type == 'o')
			&& (*lst->var != '0' || lst->pre[2]))
		opt.nb_zero = 1;
